...
class BiLSTMAttentionClassifier(base_train.BiLSTMClassifier):
    def __init__(
        self,
        vocab_size: int,
        embed_dim: int,
        ...
        self.attention_proj = nn.Linear(attention_dim, attention_dim)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        h_all = layer_input
        u = torch.tanh(self.attention_proj(h_all))
        scores = self.attention_score(u).squeeze(-1)

        pad_mask = x == self.pad_idx
        scores = scores.masked_fill(pad_mask, -1e9)
        alpha = torch.softmax(scores, dim=1)

        context = torch.sum(h_all * alpha.unsqueeze(-1), dim=1)
